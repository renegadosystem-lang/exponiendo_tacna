document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        // Nuevos selectores para la gestión de medios  
        addMediaForm: document.getElementById('add-media-form'), // Nuevo selector
        addFilesInput: document.getElementById('add-files-input'), // Nuevo selector
        addFilesStatus: document.getElementById('add-files-status') // Nuevo selector
    };
    ...
    // --- LISTENER NUEVO PARA SUBIR MÁS ARCHIVOS ---
    ...
    // --- FUNCIÓN NUEVA PARA MANEJAR LA SUBIDA DE MÁS ARCHIVOS ---
    async function handleAddMediaSubmit(e) {
        ...
        // Recargar el modal para mostrar los nuevos archivos
        loadAlbum().then(() => {
            openManagementModal();
            // Cambiar a la pestaña de archivos para ver los resultados
    ...
    async function handleFollow() {
        ...
        if (response.ok) {
            loadAlbum(); // Recargar para actualizar contadores y estado del botón
    ...
    async function handleReportSubmit(e) {
        ...
        const response = await fetchAPI(`/api/albums/${albumId}/report`, { method: 'POST', body: JSON.stringify({ reason }) });
        if (response.ok) {
    ...
    async function handleCommentAction(e) {
        ...
        if (action === 'report-comment') {
            ...
            if (reason) {
                const response = await fetchAPI(`/api/comments/${commentId}/report`, { method: 'POST', body: JSON.stringify({ reason }) });
                const data = await response.json();
    ...
    async function handleSaveMediaOrder() {
        ...
        if (response.ok) {
            ...
            // No es necesario recargar toda la página, solo actualizar el estado local
            // Para simplicidad por ahora, recargamos
            loadAlbum(); 
        } else {
    ...
    async function handleMediaManagementClick(e) {
        ...
        if(!mediaId) return; // Prevenir error si no se encuentra el ID
        ...
        if (action === 'delete-media') {
            if (confirm('¿Seguro que quieres eliminar este archivo permanentemente?')) {
                ...
                if (response.ok) {
                    mediaItemEl.remove();
                    loadAlbum(); // Recargar para actualizar contadores
                } else {
        ...
        if (action === 'set-cover') {
            ...
            if (response.ok) {
                ...
                loadAlbum(); // Recargar para que el thumbnail principal se actualice
            } else {
