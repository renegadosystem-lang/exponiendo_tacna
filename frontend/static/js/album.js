document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderComments(comments) {
        ...
        if (comments.length > 0) {
            comments.forEach(comment => {
                ...
                if (state.currentUserId && (isOwnerOfComment || isOwnerOfAlbum || (state.currentUser && state.currentUser.is_admin))) {
                    actionsMenu = `
                        <div class="comment-actions">
                            ...
                            <div class="comment-actions-menu">
                                ...
                                ${(isOwnerOfAlbum || (state.currentUser && state.currentUser.is_admin)) && !isOwnerOfComment ? '<button class="delete" data-action="delete-comment">Eliminar (Mod)</button>' : ''}
    ...
    function setupEventListeners() {
        ...
        elements.reportBtn.setAttribute('data-modal-target', '#report-album-modal');
    ...
    async function handleReportSubmit(e) {
        ...
        const response = await fetchAPI(`/api/albums/${albumId}/report`, { method: 'POST', body: JSON.stringify({ reason, description }) });
        if (response.ok) {
