// /static/js/album.js (Versión Definitiva, Completa y Funcional)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        reportCommentForm: document.getElementById('report-comment-form'),
    ...
    function flattenComments(comments, depth = 0) {
        ...
        for (const comment of sortedComments) {
            ...
            if (comment.replies && comment.replies.length > 0) {
                flatList = flatList.concat(flattenComments(comment.replies, depth + 1));
            }
        }
        return flatList;
    }
    ...
    function renderComments(comments, container) {
        ...
        if (topLevelComments && topLevelComments.length > 0) {
            const flatCommentList = flattenComments(topLevelComments);
            
            flatCommentList.forEach(comment => {
                const commentEl = document.createElement('div');
                commentEl.className = 'comment';
                if (comment.depth > 0) {
                    commentEl.classList.add('is-reply');
                }
                ...
                commentEl.innerHTML = `
                    ...
                    <span class="comment-meta">
                        por <a href="/profile.html?user=${comment.author_username}" class="profile-link"><strong>@${comment.author_username}</strong></a> - ${new Date(comment.created_at).toLocaleString('es-PE', { dateStyle: 'short', timeStyle: 'short' })}
                        ${state.isLoggedIn ? `<button class="reply-btn" data-action="show-reply-form" data-reply-to="${comment.author_username}">Responder</button>` : ''}
                    </span>
                    ${actionsMenu}
    ...
    function updateInteractionState(album) {
        ...
        if (isLoggedIn) {
            elements.mainNav.innerHTML = `<button class="nav-icon-btn" id="search-btn" data-modal-target="#search-modal" title="Buscar"><i class="fas fa-search"></i></button><button class="nav-icon-btn" id="notifications-btn" title="Notificaciones"><i class="fas fa-bell"></i><span class="notification-badge"></span></button><button class="nav-icon-btn" id="chat-btn" title="Chat"><i class="fas fa-comment-dots"></i></button><a href="/profile.html?user=${localStorage.getItem('username')}" id="my-profile-link" class="btn btn-secondary">Mi Perfil</a><a href="#" id="logout-btn" class="btn btn-secondary">Cerrar Sesión</a>`;
        } else {
            elements.mainNav.innerHTML = `<a href="/index.html#registro" class="btn btn-primary">Crear Cuenta</a><a href="/index.html#login" data-modal-target="#login-modal" class="btn btn-secondary">Iniciar Sesión</a>`;
        }
    ...
    function loadMediaForManagement(media) {
        ...
        media.forEach(item => {
            ...
            itemEl.innerHTML = `
                <div class="media-mgmt-thumbnail">${thumbnail}</div>
                <button class="media-mgmt-btn delete-btn" data-action="delete-media" title="Eliminar Archivo">🇽</button>
                <button class="media-mgmt-btn set-cover-btn" data-action="set-cover" title="Poner como Portada">🖼️</button>
            `;
    ...
    async function handleCommentAction(e) {
        ...
        if (action !== 'submit-reply') e.preventDefault();
        ...
        if (action === 'report-comment') {
            activeCommentReportId = commentEl.dataset.commentId;
            const reportModal = document.getElementById('report-comment-modal');
            if (reportModal) reportModal.classList.add('is-visible');
        }

        if (action === 'show-reply-form') {
            if (!state.isLoggedIn) return showAlert('Acceso Restringido', 'Debes iniciar sesión.');

            const openFormTextarea = document.querySelector('.reply-form-container textarea');
            if (openFormTextarea && openFormTextarea.value.trim() !== "") {
                const confirmed = await showConfirm('Descartar Borrador', 'Tienes una respuesta en progreso. ¿Deseas descartarla?');
                if (!confirmed) return;
            }
            ...
            const commentId = commentEl.dataset.commentId;
            const replyToUsername = target.dataset.replyTo;
            
            container.innerHTML = `<form class="reply-form" onsubmit="return false;"><textarea name="text" placeholder="Respondiendo a @${replyToUsername}..." required></textarea><button type="submit" class="btn btn-primary" data-action="submit-reply" data-parent-id="${commentId}">Enviar</button></form>`;
    ...
    async function handleMediaManagementClick(e) {
        ...
        if (action === 'delete-media') {
            ...
            if (confirmed) {
                ...
                if (response.ok) { 
                    ...
                    mediaItemEl.remove();
                    state.albumData.media = state.albumData.media.filter(m => m.id !== parseInt(mediaId));
        ...
        if (action === 'set-cover') {
            ...
            if (response.ok) { 
                ...
                const newCover = state.albumData.media.find(m => m.id === parseInt(mediaId));
                if(newCover) state.albumData.thumbnail_path = newCover.file_path;
            } else { showToast('Error al establecer la portada.', 'error'); }
        }
    }

    async function handleReportCommentSubmit(e) {
        e.preventDefault();
        if (!activeCommentReportId) return;

        const reason = e.target.reason.value;
        if (!reason) return showToast('Debes seleccionar una razón.', 'error');

        const response = await fetchWithAuth(`/api/comments/${activeCommentReportId}/report`, {
            method: 'POST',
        ...
        if (response.ok) {
            showToast('Comentario reportado. Gracias.');
        } else {
            const data = await response.json();
            showToast(data.message || 'Error al reportar.', 'error');
        }
        
        e.target.reset();
        if (modal) modal.classList.remove('is-visible');
        activeCommentReportId = null;
    }
    ...
    function setupEventListeners() {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === "Escape") {
                const openReplyForm = document.querySelector('.reply-form-container:not(:empty)');
                if (openReplyForm) {
                    openReplyForm.innerHTML = '';
                }
            }
        });

        if (elements.reportCommentForm) {
            elements.reportCommentForm.addEventListener('submit', handleReportCommentSubmit);
        }
    }
