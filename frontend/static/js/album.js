document.addEventListener('DOMContentLoaded', () => {
    ...
    // CORRECCIÓN 1: Lógica para renderizar comentarios simplificada y corregida.
    function renderComments(comments) {
        ...
        if (comments.length > 0) {
            comments.forEach(comment => {
                ...
                // Solo mostramos el menú de acciones si el usuario está logueado.
                if (state.currentUserId) {
                    let menuButtons = '';
                    // El dueño del comentario O el dueño del álbum pueden eliminar.
                    if (isOwnerOfComment || isOwnerOfAlbum) {
                        ...
                        // Si no es dueño de nada, puede reportar.
    ...
    // CORRECCIÓN 2: Lógica de acciones de comentario mejorada.
    async function handleCommentAction(e) {
        const target = e.target.closest('[data-action]');
        if (!target) return;

        // Ocultar todos los menús abiertos para una mejor experiencia de usuario.
        ...
        // AHORA: Se usa el modal de confirmación en lugar de un prompt, dando mejor feedback.
