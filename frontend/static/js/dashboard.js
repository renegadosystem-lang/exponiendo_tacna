document.addEventListener('DOMContentLoaded', () => {
    ...
    const createAlbumCard = (album, isOwner) => {
        ...
        // --- CORRECCIÓN CLAVE: El div principal tiene el data-album-id y no está envuelto en un <a> ---
        return `
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (target.closest('.album-owner-controls') || target.closest('.profile-link') || target.closest('button') || target.closest('.modal')) {
            e.preventDefault();
        }
        ...
        // Cerrar el modal si se hace clic fuera del contenido  (modal background)
        if (target.classList.contains('modal') && target.classList.contains('is-visible')) {
            target.classList.remove('is-visible');
        }                       

        // Si el clic es en cualquier otra parte de la tarjeta, navegar al álbum
