...
if (albumData.comments && albumData.comments.length > 0) {
    ...
    commentsList.innerHTML = '<p>No hay comentarios. ¡Sé el primero!</p>';
}
...
commentForm.onsubmit = async (e) => {
    ...
    const response = await fetchWithAuth(`/api/albums/${albumId}/comments`, {
        method: 'POST',
        body: JSON.stringify({ text })
    });
    if (response.ok) {
        e.target.reset();
        // Para ver el nuevo comentario, lo más fácil es recargar el visor
