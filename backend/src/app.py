...
def get_album(album_id):
    ...
    # --- LÓGICA CORREGIDA Y ROBUSTA PARA COMENTARIOS ---
    top_level_comments = album.comments.filter_by(parent_id=None).order_by(Comment.created_at.asc()).all()
    
    def format_comment(comment):
        # Este "if" previene el crash si el autor del comentario fue eliminado
    ...
    return jsonify({
        ...
        'user_id': album.user_id,
        'owner_username': album.owner.username,
        'owner_profile_picture': get_public_url(album.owner.profile_picture_path),
        ...
        'saves_count': album.saves_count,
        'shares_count': album.shares_count,
        'user_is_logged_in': user_is_logged_in,
        'is_followed': is_followed, 
        'is_liked': is_liked,
        'is_saved': is_saved,
        'current_user_profile_picture': current_user_profile_picture,
        'thumbnail_path': get_public_url(album.thumbnail_path)
    })
