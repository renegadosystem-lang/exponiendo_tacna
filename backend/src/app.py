...
def report_comment(comment_id):
    ...
    comment = Comment.query.get_or_404(comment_id)
    if reporter_id == comment.user_id: return jsonify({'error': 'No puedes reportar tu propio comentario'}), 403
    if CommentReport.query.filter_by(reporter_id=reporter_id, comment_id=comment_id).first(): return jsonify({'message': 'Ya has reportado este comentario.'}), 409
