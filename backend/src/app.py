...
app.config['SQLALCHEMY_DATABASE_URI'] = DATABASE_URL or f"postgresql://{os.environ.get('DB_USER')}:{os.environ.get('DB_PASSWORD')}@{os.environ.get('DB_HOST')}/{os.environ.get('DB_NAME')}"
...
# --- 3. Modelos de la Base de Datos (con relaciones back_populates) ---
...
def get_user_profile(username):
    ...
    # --- LÓGICA AÑADIDA PARA VERIFICAR SI SE SIGUE AL USUARIO ---
    is_followed = False
    ...
    if auth_header and auth_header.startswith('Bearer '):
        try:
            ...
            pass # Ignorar si el token es inválido
    ...
    return jsonify({
        ...
        'is_followed': is_followed # <-- Añadir este dato a la respuesta
    })
...
@jwt_required(optional=True) # Permite GET sin token, pero POST lo requerirá
def handle_albums():
    """
    Manejador unificado para la ruta /api/albums.
    - GET: Devuelve una lista paginada de álbumes.
    - POST: Crea un nuevo álbum (requiere autenticación).
    """
    if request.method == 'POST':
        # --- Lógica para CREAR un álbum (POST) ---
    ...
    # --- Lógica para OBTENER todos los álbumes (GET) ---
...
def reorder_media(album_id):
    ...
    # --- INICIO: LÓGICA DE PORTADA AUTOMÁTICA ---
    # Busca el primer archivo en el nuevo orden y lo establece como portada
    ...
    # --- FIN: LÓGICA DE PORTADA AUTOMÁTICA ---
...
def report_comment(comment_id):
    ...
    # CORRECCIÓN: Se elimina el filtro por 'status' ya que no existe en el modelo.
...
def report_album(album_id):
    ...
    description = data.get('description') # <-- CORRECCIÓN: Aceptar descripción
    ...
    # --- CORRECCIÓN: Incluir descripción en la creación del reporte ---
