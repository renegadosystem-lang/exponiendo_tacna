...
class Album(db.Model):
    ...
    thumbnail_path = db.Column(db.String(255), nullable=True) # Para la portada personalizada
...
class Media(db.Model):
    ...
    position = db.Column(db.Integer, default=0, nullable=False) # Para el orden
...
class Comment(db.Model):
    ...
    # Esta línea es la que crea la relación correctamente y debe quedarse
...
class CommentReport(db.Model):
    ...
    # La relación 'reporter' se queda, pero la relación 'comment' se elimina
    # porque ya se crea automáticamente desde el modelo Comment.
...
def get_user_profile(username):
    ...
    for album in user_albums:
        ...
        if album.thumbnail_path:
           thumbnail_url = supabase.storage.from_(BUCKET_NAME).get_public_url(album.thumbnail_path)
    else:
           first_media = Media.query.filter_by(album_id=album.id).order_by(Media.position.asc(), Media.created_at.asc()).first()
    ...
    return jsonify({
        ...
        'albums': albums_list,
        'followers_count': followers_count, # <-- AÑADIDO
        'following_count': following_count  # <-- AÑADIDO
    })
...
def get_album(album_id):
    ...
    # ... (código para incrementar vistas, contar media, etc. se mantiene igual) ...
    album.views_count += 1
    db.session.commit()
    ...
    # --- MODIFICADO: Añadir IDs a los comentarios ---
    comments_list = []
    for c in sorted(album.comments, key=lambda c: c.created_at, reverse=True):
        comments_list.append({
            'id': c.id, # ID del comentario
            'text': c.text,
            'author_username': c.author.username,
            'author_id': c.user_id, # ID del autor del comentario
 ...
 # --- NUEVO: Contadores de seguidores del dueño del álbum ---
    ...
    user_is_logged_in = False # <-- Inicia como False por defecto
    ...
    if auth_header and auth_header.startswith('Bearer '):
        try:
            ...
            user_is_logged_in = True # <-- SE PONE EN TRUE SI EL TOKEN ES VÁLIDO
            ...
            user_is_logged_in = False # <-- Se asegura que sea False si hay error

    return jsonify({
        'id': album.id, 'title': album.title, 'description': album.description, 
        'user_id': album.user_id, 'owner_username': album.owner.username, 'owner_profile_picture': owner_profile_picture,
        'owner_followers_count': owner_followers_count, # <-- AÑADIDO
        'owner_following_count': owner_following_count, # <-- AÑADIDO
        ...
        # --- LÍNEAS CORREGIDAS ---
        'user_is_logged_in': user_is_logged_in, # <-- La clave que faltaba
...
def set_album_cover(album_id):
    ...
    if not media_id:
        return jsonify({'error': 'Falta el ID del archivo multimedia'}), 400

    media_item = Media.query.get(media_id)
    if not media_item or media_item.album_id != album_id:
        return jsonify({'error': 'Archivo multimedia no encontrado en este álbum'}), 404
...
# --- Rutas de API: Moderación de Comentarios ---
...
def delete_comment(comment_id):
    ...
    # Permiso: El usuario es el autor del comentario O el dueño del álbum
