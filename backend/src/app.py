# =========================================================================
#  app.py (Versión Corregida, Optimizada y Completa)
# =========================================================================
import os
...
# (Sin cambios en los modelos, se mantienen igual que en tu versión)
...
# =========================================================================
# (Sin cambios en las funciones de ayuda, se mantienen igual que en tu versión)
...
#  5. RUTAS DE API
# =========================================================================
# (Rutas de auth, perfiles y subida de imágenes sin cambios)
...
# --- NUEVO ENDPOINT OPTIMIZADO PARA "MIS ÁLBUMES" ---
...
# (Rutas de búsqueda y notificaciones sin cambios)
...
def search():
    ...
    users_list = [{'username': u.username, 'profile_picture_url': get_public_url(u.profile_picture_path)} for u in users]
    ...
    for notif in notifications:
        ...
        if notif.notification_type == 'new_follower':
            ...
            if album:
                ...
                message = f"<strong>{actor_user.username}</strong> {action_text} tu álbum <strong>{album.title}</strong>."
...
# (Ruta /api/albums GET/POST sin cambios)
...
def get_album(album_id):
    ...
    top_level_comments = album.comments.filter_by(parent_id=None).order_by(Comment.created_at.asc()).all()
    
    def format_comment(comment):
        author_username = comment.author.username if comment.author else "[usuario eliminado]"
        # AQUÍ ESTÁ EL CAMBIO: Se obtiene el nombre de usuario del comentario padre
        replying_to = comment.parent.author.username if comment.parent and comment.parent.author else None
        ...
        return {
            ...
            'replying_to': replying_to, # El nuevo campo que el frontend necesita
            'replies': [format_comment(reply) for reply in sorted(comment.replies, key=lambda r: r.created_at)]
        }
...
# (Resto de las rutas sin cambios significativos)
